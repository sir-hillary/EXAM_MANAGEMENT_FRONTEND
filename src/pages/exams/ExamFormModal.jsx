...
import { useSubjectsForClass } from "../../hooks/useSubjects";
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-3.5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <SelectField
            ...
            onChange={(e) => {
              register("subject_id").onChange(e);
              // Auto-set total_marks when a paper subject is selected
              const subjectId = e.target.value;
              const subject = subjectsForDivision?.data?.find(
                (s) => String(s.id) === String(subjectId),
              );
              if (subject?.max_marks) {
                setValue("total_marks", subject.max_marks);
              }
            }}
          >
            <option value="">
              {selectedClassId ? "Select subject..." : "Select a class first"}
            </option>
            {(subjectsForDivision?.data || []).map((s) => (
              <option key={s.id} value={s.id}>
                {s.name} {s.paper_number ? `(max ${s.max_marks})` : ""}
              </option>
            ))}
          </SelectField>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          >
            <option value="">
              {selectedSubjectId ? "Unassigned" : "Select a subject first"}
            </option>
