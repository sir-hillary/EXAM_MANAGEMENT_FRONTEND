...
export default function TermReportCard() {
  ...
  const { data: studentsData } = useStudents({ limit: 200 }, { enabled: !isStudent });
  const { data: reportData, isLoading, isError, error } =
  ...
  return (
    <div>
      <PageHeader
        ...
        description={isStudent ? 'Your term academic report' : "Generate a student's term report card"}
        action={report && (
          <button onClick={handleDownload} disabled={downloading} className="btn-primary w-full sm:w-auto justify-center">
            {downloading
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        {!isStudent && (
          <div className="sm:w-72">
            <SelectField label="Student" value={selectedStudentId} onChange={(e) => setSelectedStudentId(e.target.value)}>
      ...
      {!selectedStudentId ? (
        <div className="bg-white border border-gray-200 rounded-lg py-12 text-center text-sm text-gray-500">
          {isStudent ? 'Loading your report...' : 'Select a student to generate their term report card'}
        </div>
      ) : isLoading ? (
        ...
        <>
          ...
          <div className="overflow-x-auto rounded-lg shadow-sm border border-gray-200">
            <div ref={documentRef} style={{ isolation: 'isolate', all: 'initial', display: 'block', colorScheme: 'light' }}>
              <ReportCardDocument
                report={report}
